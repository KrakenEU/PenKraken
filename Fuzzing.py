    ...
    def post_fuzzing(self):
        ...
        if login == '1':
            ...
            if 'y' in json:
                ...
                command = f"ffuf -w {user_wordlist}:USERFUZZ -w {pass_wordlist}:PASSFUZZ -u {self.target} -X POST -d '{user_payload}=USERFUZZ&{pass_payload}=PASSFUZZ' -fr {filter_ex}"
                process = subprocess.Popen(command, stdout=subprocess.PIPE, stderr=subprocess.STDOUT,text=True, shell=True)
                while True:
                    salida = process.stdout.readline()
                    if salida == '' and process.poll() is not None:
