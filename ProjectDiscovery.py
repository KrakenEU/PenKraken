    ...
    def auto_scan(self):
        try:
            ...
            for x in str(output.decode()).split('\n'):
                print(f"{penkraken.colors['magenta']} {x} {penkraken.colors['reset']}")
                self.results.append(x)
            print(f"{penkraken.colors['blue']}\n[+] Running nuclei http scan on all subdomains (This could take time...){penkraken.colors['reset']}")
            nc_output = subprocess.check_output(f"nuclei -list {t}-urls.txt -t http/technologies -t http/exposures -t http/vulnerabilities -t http/vulnerabilities -t http/fuzzing", shell=True)
            # print and save results
